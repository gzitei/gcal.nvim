...
local function parse_event_times(event)
  ...
  local end_str = event["end"] and (event["end"].dateTime or event["end"].date)
...
local function deduplicate_events(events, calendars)
  ...
  for _, ev in ipairs(events) do
    ...
    if not uid then
      ...
      local start_date = (ev.start and (ev.start.dateTime or ev.start.date)) or ""
      ...
      if not existing_idx then
        ...
        if not primary_ids[current_winner._calendar_id] and primary_ids[ev._calendar_id] then
...
local function build_all_day_lines(all_day_events, week_start_ts, col_w)
  ...
  for row_idx, key in ipairs(order) do
    ...
    for d, _ in pairs(g.days) do
      if d < min_day then min_day = d end
...
local function render(events, calendars)
  ...
  local grid_lines, overlap_hints = build_grid(events_by_slot, col_w, total_slots, slot_count)
  ...
  if has_all_day then
    ...
    local all_day_section_header = string.rep(" ", left_pad) .. label .. string.rep(" ", total_width - left_pad - #label)
  ...
  vim.api.nvim_buf_add_highlight(state.buf, state.ns, "GcalHeader", header_row, 0, -1)
  ...
  if today_day >= 0 and today_day <= 6 then
    ...
    vim.api.nvim_buf_add_highlight(state.buf, state.ns, "GcalTodayHeader", header_row, cs, ce)
    for slot = 0, total_slots - 1 do
      ...
      if row < vim.api.nvim_buf_line_count(state.buf) then
        ...
        if not slot_event_data[key] then
          vim.api.nvim_buf_add_highlight(state.buf, state.ns, "GcalToday", row, cs, ce)
        end
      end
    end
  end
  ...
  if current_slot >= 0 and current_slot < total_slots then
    ...
    if now_row < vim.api.nvim_buf_line_count(state.buf) then
      vim.api.nvim_buf_add_highlight(state.buf, state.ns, "GcalNowLine", now_row, 0, -1)
    end
  end
  ...
  for slot = 0, total_slots - 1 do
    local row = grid_start_row + slot
    if row < vim.api.nvim_buf_line_count(state.buf) then
      vim.api.nvim_buf_add_highlight(state.buf, state.ns, "GcalTimeCol", row, 0, TIME_COL_WIDTH)
      for day = 0, 6 do
        local key = day .. ":" .. slot
        ...
        if ev then
          ...
          vim.api.nvim_buf_add_highlight(state.buf, state.ns, hl, row, cs, ce)
        end
      end
      ...
      if hints then
        for _, h in ipairs(hints) do
          ...
          vim.api.nvim_buf_add_highlight(state.buf, state.ns, ohl, row, h.byte_start, h.byte_end)
        end
      end
    end
  end
  ...
  if has_all_day then
    ...
    vim.api.nvim_buf_add_highlight(state.buf, state.ns, "GcalHeader", section_header_row, 0, -1)
    ...
    for entry_idx, entry in ipairs(all_day_lines) do
      ...
      if all_day_row < vim.api.nvim_buf_line_count(state.buf) then
        for _, hl_spec in ipairs(entry.highlights) do
          vim.api.nvim_buf_add_highlight(state.buf, state.ns, hl_spec[1], all_day_row, hl_spec[2], hl_spec[3])
        end
      end
    end
  end
...
local function fetch_and_render(force)
  ...
  api.get_all_events(state.current_week_start, week_end_ts, function(events, calendars)
    render(events, calendars)
  end, { cache_key = cache_key, cache_ttl = cache_ttl, force = force, once = once })
end
...
local function open_url(raw_event)
  ...
  if not url then
    ...
    vim.notify("No URL for " .. title, vim.log.levels.INFO, { title = "gcal.nvim" })
    return
  end
  if vim.ui.open then
    vim.ui.open(url)
...
local function open_event_popup(ev)
  ...
  local display_end_ts = (is_all_day and end_ts) and (end_ts - 86400) or end_ts

  local popup_width = math.min(80, math.max(40, math.floor(total_width * 0.45)))
  ...
  if start_ts then
    if is_all_day then
      ...
      if display_end_ts and display_end_ts > start_ts then
        date_str = date_str .. " - " .. os.date("%a %b %d", display_end_ts)
      end
  ...
  if start_ts and (end_ts or display_end_ts) then
    table.insert(detail_lines, "**Status:** " .. format_time_until(start_ts, display_end_ts or end_ts))
  end
  ...
  vim.keymap.set("n", "q", close_popup, { buffer = detail_buf, nowait = true })
  vim.keymap.set("n", "<Esc>", close_popup, { buffer = detail_buf, nowait = true })
  vim.keymap.set("n", "K", close_popup, { buffer = detail_buf, nowait = true })
  vim.keymap.set("n", "<CR>", function()
    close_popup()
    open_url(ev)
  end, { buffer = detail_buf, nowait = true, desc = "Open event in browser" })
...
local function go_to_today()
  state.current_week_start = utils.get_week_bounds(os.time(), config.options.week_start)
  fetch_and_render()
end
...
local function event_ts_under_cursor()
  ...
  local day = math.floor((col - TIME_COL_WIDTH - 1) / (col_w + 1))
  if day < 0 or day > 6 then return nil end
  for _, ev in ipairs(state.events) do
    local p = parse_event_times(ev)
    if p and not p.all_day then
      local d = get_day_index(p.start_ts, state.current_week_start)
...
local function update_cursor_highlight()
  if not is_valid_buf() then return end
  vim.api.nvim_buf_clear_namespace(state.buf, state.cursor_ns, 0, -1)
  ...
  local slot = row - grid_start - 1
  if slot < 0 or slot >= total_slots then return end

  local day = math.floor((col - TIME_COL_WIDTH - 1) / (col_w + 1))
  if day < 0 or day > 6 then return end
        ...
        for slot_i = s, e - 1 do
          if slot_i >= 0 and slot_i < total_slots then
            ...
            vim.api.nvim_buf_add_highlight(
              state.buf, state.cursor_ns, "GcalCursorEvent", buf_row, cs, ce
            )
          end
        end
        break
...
local function navigate_event(dir)
  ...
  if ref_idx then
    ...
    if p then ref_ts = p.start_ts end
  end
  if not ref_ts then
  ...
  if dir == 1 then
    ...
    if not target_entry then
      ...
      api.get_all_events(next_week_start, next_week_end, function(events, calendars)
        local first = nil
        ...
        if not first then
          vim.notify("No more events", vim.log.levels.INFO, { title = "gcal.nvim" })
          return
        end
        state.current_week_start = next_week_start
        render(events, calendars)
        vim.schedule(function() M._place_cursor_on(first) end)
      end, { cache_key = "next_week", cache_ttl = (config.options.poll_interval_seconds or 60) * 10 })
      return
    end
  else
    if ref_entry_pos then
    ...
    if not target_entry then
      ...
      api.get_all_events(prev_week_start, prev_week_end, function(events, calendars)
        local last = nil
        for _, ev in ipairs(events or {}) do
          local p = parse_event_times(ev)
          if p and not p.all_day and p.start_ts then
        ...
        if not last then
          vim.notify("No previous events", vim.log.levels.INFO, { title = "gcal.nvim" })
          return
        end
        state.current_week_start = prev_week_start
        render(events, calendars)
        vim.schedule(function() M._place_cursor_on(last) end)
      end)
      return
    end
  end
...
function M.goto_current_meeting()
  ...
  api.get_all_events(week_start, week_end, function(events)
    ...
    vim.ui.select(candidates, {
      prompt = "Join meeting:",
      format_item = function(p)
        local time = utils.format_time(p.start_ts, config.options.time_format)
          .. " - "
          .. utils.format_time(p.end_ts, config.options.time_format)
        local status = (now >= p.start_ts and now < p.end_ts) and " [now]" or " [next]"
  ...
  end, { cache_key = "current_week", cache_ttl = config.options.poll_interval_seconds or 60, once = true })
end
...
function M._place_cursor_on(p)
  ...
  if end_slot <= start_slot then end_slot = start_slot + 1 end
